...
//                   onChangePositiveDataset: widget.onChangePositiveDataset,
//                   onChangeNegativeDataset: widget.onChangeNegativeDataset,
//                   positiveData: widget.positiveData,
//                   negativeData: widget.negativeData,
//                 ),
//               ],
//             ),
//           );
...
class _BaseCollectionState extends ConsumerState<BaseCollection> {
  ...
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  CameraController? _cameraController;
  bool _isCollectingPositive = true;

  List<List<List<double>>> _positiveData = [];
  List<List<List<double>>> _negativeData = [];
...
// IMPORTANT CONFIGURATIONS---------------------------------------------------------------------------------------
  ...
  void initState() {
    ...
    noMovementBufferThreshold = noMovementBufferThresholdDefault;
    changeRange = changeRangeDefault;
    _positiveData = widget.positiveData;
    _negativeData = widget.negativeData;
    _positiveData.add(dummyDataset);
    _positiveData.add(dummyDataset);
    _positiveData.add(dummyDataset);
    _positiveData.add(dummyDataset);
    _positiveData.add(dummyDataset);
    ...
    _negativeData.add(dummyDataset);
    _negativeData.add(dummyDataset);
    _negativeData.add(dummyDataset);
    _negativeData.add(dummyDataset);
    _negativeData.add(dummyDataset);
  ...
  void _onChangeNoMovementBuffer(int bufferCount) {
    setState(() {
      noMovementBufferThreshold = bufferCount;
    });
  }

  void _onChangeChangeRange(double changeRangeCount) {
    setState(() {
      changeRange = changeRangeCount;
    });
  }

  void _onChangeShowPose(bool showPoseValue) {
    print("pose show adjust");
  ...
  void _onChanegShowPreview(bool showPreviewValue) {
    print("preview showing adjust");
    setState(() {
      showVideoPreview = showPreviewValue;
    });
  ...
  void _onChangeState(bool performing) {
    print("now performing");
    setState(() {
  ...
  void _onChangePause(bool isPause) {
    print("preview showing adjust");
    setState(() {
      _isPause = isPause;
    });
  ...
  void _onChangeCollectingPositive(bool isCollecting) {
    print("preview showing adjust");
    setState(() {
      _isCollectingPositive = isCollecting;
    });
  }
      ...
      if (queueMovementData.isNotEmpty) {
        compute(checkMovement, queueMovementData.elementAt(0)).then(
          (value) async {
              ...
              if (nowCollecting == true) {
                if (value == true) {
                  ...
                  if (inferencingList.isNotEmpty &&
                    ...
                    // if (ref.read(isCollectingCorrect) == true) {
                    if (_isCollectingPositive == true) {
                      // ref
                      //     .read(coordinatesDataProvider)
                      //     .addItem(inferencingList);
                      _positiveData.add(inferencingList);
                      ...
                      //     .read(incorrectCoordinatesDataProvider)
                      //     .addItem(inferencingList);
                      _negativeData.add(inferencingList);
                      // ref.read(numExecNegative.notifier).state++;
                    }
            ...
            // if no movement and not performing yet
            if (value == true && _isPerforming == false) {
              // if countdown has not yet started then start it
              if (countDowntoPerform == false) {
                _countDownController.start();
                countDowntoPerform = true;
          ...
          return Scaffold(
            body: Stack(
              children: [
                ...
                DatasetCollection(
                  onChangeNoMovementBuffer: _onChangeNoMovementBuffer,
                  onChangeChangeRange: _onChangeChangeRange,
                  onChangeShowPose: _onChangeShowPose,
                  onChanegShowPreview: _onChanegShowPreview,
                  onChangePause: _onChangePause,
                  onChangeCollectingPositive: _onChangeCollectingPositive,
                  onChangeState: _onChangeState,
                  onInitDatasetCalc: widget.onInitDatasetCalc,
                  isRetraining: widget.isRetraining,
                  isNewRetrain: widget.isNewRetrain,
                  positiveData: widget.positiveData,
                  negativeData: widget.negativeData,
                  isCollectingPositive: _isCollectingPositive,
                ),
              ],
            ),
          );
