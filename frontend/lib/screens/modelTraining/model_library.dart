...
import 'package:frontend/screens/modelTraining/model_card.dart';
import 'package:frontend/services/mainAPI.dart';
import 'package:frontend/services/model.dart';
import 'package:frontend/services/training.dart';
...
class ModelProgress extends ConsumerStatefulWidget {
  const ModelProgress({
    super.key,
  });

  @override
  ConsumerState<ModelProgress> createState() => _ModelProgressState();
}

class _ModelProgressState extends ConsumerState<ModelProgress> {
  int upperState = 0;
  int filterState = 0;
...
// Should make a one file for this to be referred by others
  ...
  List<TrainingProgress> _trainingProgress = [];

  final PageController _pageController = PageController();


  Future<void> getModelProgress() async {
    _trainingProgress = await ModelTrainingService.getUserTrainingProgress();
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: _trainingProgress.map(
                (training) {
                  print("training.taskId, --> ${training.taskId}");
        ...
        Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: _trainingProgress.map(
                (training) {
                  if (training.exercise.account.toString() == setup.id) {
                    return Container(
                      width: MediaQuery.of(context).size.width * 0.90,
                      margin: const EdgeInsets.symmetric(horizontal: 5.0),
                      child: Stack(
